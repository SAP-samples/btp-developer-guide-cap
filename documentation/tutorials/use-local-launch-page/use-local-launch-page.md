...
> [!TIP]
> The `additionalInformation` property contains `SAPUI5.Component=ns.incidents`. The `ns` part is the namespace you chose when generating the SAP Fiori application. Make sure it matches the namespace in your `app/incidents/webapp/manifest.json` file under the `sap.app > id` field.

4. Go to the tab of the opened SAP Fiori application in your browser and replace **/incidents/webapp/index.html?sap-ui-xx-viewCache=false** with **/launchpage.html#Shell-home** in the URL.
...
> [!TIP]
> The `additionalInformation` property uses `SAPUI5.Component=ns.incidents` where `ns` is the namespace you chose when generating the SAP Fiori application. Make sure this matches the `sap.app > id` in your `app/incidents/webapp/manifest.json`.
